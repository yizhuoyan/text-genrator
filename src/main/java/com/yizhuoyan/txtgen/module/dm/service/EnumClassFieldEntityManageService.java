package com.yizhuoyan.txtgen.module.dm.service;

import com.yizhuoyan.txtgen.module.dm.ao.EnumClassFieldAddOrModAo;
import com.yizhuoyan.txtgen.module.dm.entity.EnumClassFieldEntity;
import org.springframework.validation.annotation.Validated;

import javax.validation.Valid;
import java.util.List;

@Validated
public interface EnumClassFieldEntityManageService {

    List<EnumClassFieldEntity> list(Long classId)throws Exception;

    void addOrMod(@Valid  List<EnumClassFieldAddOrModAo> aos)throws Exception;

    void remove(Long... ids)throws Exception;
}
